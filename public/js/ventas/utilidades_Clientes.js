...
function Utilidades_Tabs () {
	...
	this.cambiarModal = function(){
		$('#modal_generico').modal('hide');
      	...
      	$('#tab1').fadeIn(500);
      	$('#buscaClientes').show();
      	Util_V.autoPaginacionClientes('autoPaginarClientes');
	}
...
function Utilidades_AJAX () {
	...
	this.agregarCliente = function (form,alert,url) {
		...
		if ($(form).valid()){
			...
			var cliente_data = $(form).serializeObject();
	...
	//	--	Paginado Cliente (Usando Paginador)
	this.paginandoClientes = function (pagina,url) {
	...
	//	--	Modificar Cliente
	this.modificarCliente = function(form,campos,modal,url){
		//	--	Validando Formulario
		if($(form).valid()){
			//	--	Recolectamos los datos del Formulario
			var cliente_data = $(campos).serializeObject();
			//	--	Petición Ajax(ModificarCliente)
			setTimeout
			$.ajax({
			  url: url,
			  type: 'POST',
			  ...
			  success: function(data, textStatus, xhr) {
			  	if (xhr.status == 200){
			  		...
			  		if (Util_T.leerCookie('busqueda') != null){
			  			Util_T.autoPaginacionClientes('paginarResultados',Util_T.leerCookie('busqueda'));
			  		}else{
			  			Util_T.autoPaginacionClientes('autoPaginarClientes');
			  		}
			  	}else{
			  		Util_T.mostrarModal(data);
			  		Util_T.limpiarModificarCliente(modal);
			  	}
			  },
			  error: function() { 
			  	Util_T.mostrarError(); 
	...
	//	--	Eliminar Cliente
	this.eliminarCliente = function(id,url){
		//	--	Petición Ajax
		$.ajax({
		  url: url,
		  type: 'POST',
...
function Utilidades_Formularios () {
	...
	//	--	Validar el Formulario de Modificar Cliente
	this.formModificarCliente = function(formulario) {
		$(formulario).validate({
		    rules: {
		      nombre: 		{required: true},
		      rfc: 			{required: true,accept: "[a-zA-Z0-9]+",minlength: 13,maxlength: 13},
...
function Utilidades_Varias(){
	...
	//	--	Limpiar el formulario Agregar Cliente
	this.limpiarAgregarCliente = function(){
	    ...
	    //	--	Restablecer Alerta
	    $('#alerta_registrarcliente').attr('class', 'row-fluid alert alert-info span11');
		$('#alerta_registrarcliente')
	...
	//	--	Limpiar el formulario Modificar Cliente
	this.limpiarModificarCliente = function(form,edo){
	...
	//	--	Leer una Cookie
  	...
  	this.autoPaginacionClientes =  function(url,param){
  	...
  	//	--	Paginar Desde Cero
  	this.autoPaginacionClientesInit =  function(url){
