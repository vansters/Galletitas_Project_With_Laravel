...
function Utilidades_Tabs () {
	...
	this.cambiarAgregar = function(){
		$('#tab1').hide();
		$('#tab2').fadeIn(500);
		$('#buscaProveedores').hide();
	}
	//	--	Cambiar de Agregar a Gestión
	this.cambiarGestion =  function() {
		$('#buscaProveedores').show();
		$('#tab2').hide();
		$('#tab1').fadeIn(500);
		Util_V.limpiarAgregarProveedor();
	}
	...
	this.cambiarModal = function(){
		$('#modal_generico').modal('hide');
      	...
      	$('#tab1').fadeIn(500);
      	$('#buscaProveedores').show();
      	Util_V.autoPaginacionProveedores('autoPaginarProveedores');
	}
...
function Utilidades_AJAX () {
	...
	this.agregarProveedor = function (form,alert,url) {
		//	--	Validamos el Formulario
		if ($(form).valid()){
			//	--	Recolectamos los datos del Formulario
			var proveedor_data = $(form).serializeObject();
			//	--	Petición Ajax(AgregarProveedor)
			$.ajax({
			  url: url,
			  type: 'POST',
			  dataType: 'json',
			  ...
			  success: function(data, textStatus, xhr) {
			    if (xhr.status == 200){
			    	...
			    	Util_T.limpiarAgregarProveedor();
			...
			Util_T.errorFormulario('#alerta_registrarproveedor');
		}
	}//	--	Fin

	//	--	Paginado Proveedor (Usando Paginador)
	this.paginandoProveedores = function (pagina,url) {
	...
	//	--	Modificar Proveedor
	this.modificarProveedor = function(form,campos,modal,url){
		//	--	Validando Formulario
		if($(form).valid()){
			//	--	Recolectamos los datos del Formulario
			var proveedor_data = $(campos).serializeObject();
			//	--	Petición Ajax(ModificarProveedor)
			$.ajax({
			  url: url,
			  type: 'POST',
			  dataType: 'json',
			  ...
			  success: function(data, textStatus, xhr) {
			  	if (xhr.status == 200){
			  		...
			  		if (Util_T.leerCookie('busqueda') != null){
			  			Util_T.autoPaginacionProveedores('paginarResultados',Util_T.leerCookie('busqueda'));
			  		}else{
			  			Util_T.autoPaginacionProveedores('autoPaginarProveedores');
			  		}
			  	}else{
			  		Util_T.mostrarModal(data);
			  		Util_T.limpiarModificarProveedor(modal);
			  	}
			  },
			  error: function() { 
			  	Util_T.mostrarError(); 
	...
	//	--	Eliminar Proveedor
	this.eliminarProveedor = function(id,url){
		//	--	Petición Ajax
		$.ajax({
		  url: url,
		  type: 'POST',
...
function Utilidades_Formularios () {
	...
	//  --  Validando Formulario de Registro Nuevo Proveedor
	$('#form_agregarProveedor').validate({
	    rules: {
	...
	//	--	Validar el Formulario de Modificar Proveedor
	this.formModificarProveedor = function(formulario) {
		$(formulario).validate({
		    rules: {
		      nombre: 		{required: true},
		      rfc: 			{required: true,accept: "[a-zA-Z0-9]+",minlength: 13,maxlength: 13},
...
function Utilidades_Varias(){
	...
	//	--	Limpiar el formulario Agregar Proveedor
	this.limpiarAgregarProveedor = function(){
	    ...
	    //	--	Restablecer Alerta
	    $('#alerta_registrarproveedor').attr('class', 'row-fluid alert alert-info span11');
		$('#alerta_registrarproveedor')
	...
	//	--	Limpiar el formulario Modificar Proveedor
	this.limpiarModificarProveedor = function(form,edo){
	...
	//	--	Leer una Cookie
  	...
  	this.autoPaginacionProveedores =  function(url,param){
  	...
  	//	--	Paginar Desde Cero
  	this.autoPaginacionProveedoresInit =  function(url){
